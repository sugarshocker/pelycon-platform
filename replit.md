...
- **Fully Loaded Cost & Margin**: Total cost = labor cost + agreement addition costs. Labor from ConnectWise `/time/entries` (last 12mo), grouped by engineer with `hourlyCost` from `/system/members/{id}` (cached). Addition costs from `/finance/agreements/{id}/additions` for each non-cancelled agreement — annualized (qty × unitCost × 12). Margin = (revenue - total cost) / revenue. Margin color thresholds: green ≥70%, yellow ≥55%, orange <55%.
- **Agreement Addition Tracking**: Per-addition detail showing product name, agreement, qty, unit cost/price, annual cost, and per-addition margin. Low-margin (<20%) and unbilled (cost but $0 revenue) additions flagged.
- **Margin Analysis Engine**: Rule-based analysis generates actionable insights stored in `marginAnalysis` jsonb. Computed during sync and on-the-fly for existing data. Analysis checks: overall margin (<55% warning, <70% suggestion), expensive top engineer concentration (>40% of cost at $80+/hr), senior engineer utilization ($100+/hr >30% of cost), low project margin (<50%), unbilled project work (hours logged but $0 invoiced), agreement service margin pressure (<50% after additions), low-margin/unbilled agreement additions.
- **Cost & Margin Breakdown Dialog**: Dedicated people icon button per row. Tabbed dialog: Overview (revenue/cost/margin stats, cost breakdown bar, key concerns), Engineers (per-engineer hours/cost table), Additions (agreement addition detail table), Analysis (all margin insights with type-coded cards: warnings, suggestions, info).
- **Schema Columns**: `laborCost`, `additionCost`, `totalCost`, `serviceHours`, `projectHours`, `totalHours`, `engineerBreakdown` (jsonb), `agreementAdditions` (jsonb), `marginAnalysis` (jsonb)
